        ...
        <div className="mb-4">
          ...
          {options.length === 0 ? (
            <div className="text-center py-4 bg-gray-50 rounded-md">
              <p className="text-gray-500">
                No {question.type === "multiple-choice" ? "options" : "items"}{" "}
                added yet. Click "Add{" "}
                {question.type === "multiple-choice" ? "Option" : "Item"}" to
                add one.
              </p>
            </div>
          ) : (
              ...
              {options.map((option, index) => (
                <div key={option.id} className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    placeholder={`${
                      question.type === "multiple-choice" ? "Option" : "Item"
                    } ${index + 1}`}
                  />
                  <button
                    type="button"
          ...
          {question.type === "multiple-choice" && (
            <p className="mt-2 text-sm text-gray-500">
              Check the box next to correct options.
            </p>
          )}

          {question.type === "drag-drop" && (
            <p className="mt-2 text-sm text-gray-500">
              Items will be presented in random order. The order you add them
